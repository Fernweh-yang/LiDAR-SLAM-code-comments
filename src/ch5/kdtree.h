...
// 如果分割轴为第一个轴，阈值为0.5。那么x<0.5将位于左子树，x>=0.5的点位于右子树。
struct KdTreeNode {
    int id_ = -1;                  // 节点id：方便使用id来索引这棵树，而不需要写递归代码来索引了
    int point_idx_ = 0;            // 点的索引
    int axis_index_ = 0;           // 分割轴
    float split_thresh_ = 0.0;     // 分割位置(阈值)
...
class KdTree {
    ...
    private:
        ...
        int k_ = 5;                                   // knn最近邻数量
        std::shared_ptr<KdTreeNode> root_ = nullptr;  // 根节点 ： 让kd树持有根节点的指针，这样就可以遍历整棵树
